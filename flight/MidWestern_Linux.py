    ...
    def storeData(self,id):
        ...
        while st==0:
            ...
            try:
                while True:
                    c=pickle.load(c1)
                    if c.cid==self.cid and self.cid!=id:
...
class Flight:
    ...
    def storeData(self,num):
        ...
        while st==0:
            ...
            try:
                while True:
                    f=pickle.load(f1)
                    if f.fnumber==self.fnumber and self.fnumber!=num:
    ...
    def displayFlightShort(self):
        ...
        print('From: {}\tTo: {}'.format(self.fdepart,self.farrive))
    ...
    def displayFlight(self):
        self.displayFlightShort()
        print('Number of Economy seats left: ',self.feconomy)
        print('Economy Class seats available: ',self.feconomynos)
        print('First Class fare: USD ',self.ffirstprice)
        print('Number of First seats left: ',self.ffirst)
        print('First Class seats available: ',self.ffirstnos)
        print('Business Class fare: USD ',self.fbusinessprice)
        print('Number of Business seats left: ',self.fbusiness)
        print('Business Class seats available: ',self.fbusinessnos)
        print()
...
class Booking:
    ...
    def storeBooking(self,fnum,cusname):
        ...
        if status1==0:
            ...
            try:
                ...
                self.bdate=Ctime[4:7].upper()+Ctime[8:10]+Ctime[-4:]
    ...
    def displayBooking(self):
        ...
        if self.btotalpriceUSD!=self.btotalpriceLocal:
            x,to_curr,to_curr_name=self.currency_local()
    ...
    try:
        while True:
            ...
            if b.bid==id:
                    ...
                    while True:
                        ...
                        if f.fnumber==b.bflightnum:
                            if b.bclass=='Business Class':
                                ...
                                f.fbusinessnos.extend(b.bseatnumbers)
                                f.fbusinessnos.sort()
                            elif b.bclass=='First Class':
                                f.ffirst+=b.btotalseats
                                f.ffirstnos.extend(b.bseatnumbers)
                                f.ffirstnos.sort()
                            else:
                                f.feconomy+=b.btotalseats
                                f.feconomynos.extend(b.bseatnumbers)
                                f.feconomynos.sort()
                        pickle.dump(f,f2)
                except EOFError:
                    f1.close()
                ...
                f2.close()
                os.remove("flight.dat")
                os.rename("newfile2.dat","flight.dat")
...
def customer_menu(name):
    ...
    if c_choice==1:
        ...
        redirecting()
        editProfile(name)
        print('\nProfile info changed! Log in again to continue.\n')
        redirecting()
...
def current_time():
    try:
        ...
        print('\nYou are not connected to the internet! Please try again with a good connection...\n')
        exit()
...
def location():
    try:
        ...
        print('\nPoor internet connection! Please try again with a good connection...\n')
        exit()
